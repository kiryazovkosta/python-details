...
class Profile:
    ...
    def password(self, value):
        valid_digit_rule =  any(c for c in value if c.isdigit())
        valid_upper_rule = any(c for c in value if c.isupper())

        if len(value) < 8 or valid_digit_rule == False or valid_upper_rule == False:
            raise ValueError("The password must be 8 or more characters with at least 1 digit and 1 uppercase letter.")

        self.__password = value

    def __str__(self):
        return f'You have a profile with username: "{self.username}" and password: {Profile.to_stars(self.password)}'

    @staticmethod
    def to_stars(value):
        return ''.join(['*' for p in list(value)])
